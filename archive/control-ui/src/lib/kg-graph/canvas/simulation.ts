...
export class ForceSimulation {
		...
		try {
			...
			this.sim.force(
				...
				d3
					...
					.strength((link) => {
						if (link.edgeType === "derives") return FORCE_CONFIG.linkStrength.docMemory;
						if (link.edgeType === "updates") return FORCE_CONFIG.linkStrength.version;
	...
	update(nodes: GraphNode[], edges: GraphEdge[]): void {
		...
		if (linkForce) linkForce.links(edges.filter((e) => e.edgeType !== "extends"));
	}

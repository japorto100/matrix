...
import type { DocumentNodeData, GraphNode, GraphThemeColors, MemoryNodeData } from "../types";
...
function KeyBadge({ children, colors }: { children: React.ReactNode; colors: GraphThemeColors }) {
		...
		<div style={containerStyle}>
			{chain.map((entry) => {
				...
				const textStyle: React.CSSProperties = {
					fontSize: 11,
					lineHeight: 1.35,
					color: isCurrent ? colors.popoverTextPrimary : colors.popoverTextMuted,
				};
...
export const NodeHoverPopover = memo<NodeHoverPopoverProps>(function NodeHoverPopover({
	node,
	screenX,
	screenY,
	nodeRadius,
	...
	const hasForgetInfo = memoryMeta && (memoryMeta.isForgotten || memoryMeta.forgetAfter);
	...
	const { popoverX, popoverY, connectorPath } = useMemo(() => {
		...
		const quadrant = pickBestQuadrant(screenX, screenY, nodeRadius, cw, ch, TOTAL_W + GAP, TOTAL_H);
				...
				<div style={cardStyle}>
					{hasChain ? (
						...
						<div style={contentPadStyle}>
							<p style={contentTextStyle}>{truncate(content, 100) || "No content"}</p>
						</div>
					)}
					...
					{memoryMeta && hasForgetInfo && (
						...
						>
							{memoryMeta.forgetAfter && (
								...
								>
									Expires: {new Date(memoryMeta.forgetAfter).toLocaleDateString()}
								</span>
							)}

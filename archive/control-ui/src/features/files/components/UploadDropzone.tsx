...
async function uploadFile(file: File): Promise<{ fileId: string }> {
	...
	const { upload_url, file_id } = (await intentRes.json()) as {
		upload_url: string;
		file_id?: string;
	};
...
interface UploadDropzoneProps {
	/** Called after each successful upload. Receives the backend file_id for downstream actions (e.g. ingestion trigger). */
	onUploaded?: (info: { filename: string; fileId: string }) => void;
}
			...
			await Promise.allSettled(
				accepted.map(async (file) => {
					try {
						...
						onUploaded?.({ filename: file.name, fileId });
					} catch (err) {
						setResults((prev) =>
							prev.map((r) =>

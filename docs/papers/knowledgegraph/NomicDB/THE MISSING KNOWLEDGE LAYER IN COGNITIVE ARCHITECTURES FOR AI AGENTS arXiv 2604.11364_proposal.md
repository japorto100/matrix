# [PROPOSAL] - Abstracted Memory model as policy

**Source:** https://github.com/orneryd/NornicDB/issues/100  
**Author:** @orneryd  
**Created:** 2026-04-15T21:34:03Z  
**State:** open  
**Labels:** enhancement, Feedback Wanted  
**Related paper:** [arXiv 2604.11364 — The Missing Knowledge Layer in Cognitive Architectures for AI Agents](https://arxiv.org/pdf/2604.11364)
...
### @orneryd — 2026-04-16T17:19:22Z

Updated the specification a bit.

### @orneryd — 2026-04-17T17:12:38Z

I added promotion tiers as declarative polices to the spec. This keeps the promotion and decay in the same system; sort of like constraints. However, decay and promotion operate in opposition to one another. I think this gives us a layered approach to how we consider the memory dynamics where even when promoted, a decaying node can still have properties that decay independently from the rest of the node. 



### @orneryd — 2026-04-17T17:29:05Z

added plans about the interaction with MVCC and which version to score time from, i just put two there, but maybe we should make it more flexible with like a T-`N` where `N` is the number of versions back so it would be clamped to the create time if there weren't `N` versions to lookup

### @orneryd — 2026-04-17T17:30:59Z

This is a big feature and would likely change the version to a 1.1.0 release and not be backwards compatible with older memory models. Or we leave the legacy model for now with a feature flag but eventually we would still deprecate and remove the old memory management system. but I'd love thoughts from the community.

### @orneryd — 2026-04-17T17:53:08Z

I am also thinking that the workaround for now with the existing memory tiers is to utilize the retention policy that I forgot to finish implementing, now I will finish that and also add a way to exclude labels from retention policies. That would give you the 4th memory tier and deletion of things old enough not to keep around. it's a cheap hack vs the full declarative implementaiton but should satisfy the researcher's concerns I think.

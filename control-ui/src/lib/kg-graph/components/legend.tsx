...
function HexagonIcon({ fill, stroke, size = 12 }: { fill: string; stroke: string; size?: number }) {
	return (
		<svg
			aria-hidden="true"
			height={size}
...
function LineIcon({ color, dashed = false }: { color: string; dashed?: boolean }) {
	return (
		<div
			style={{
				width: 12,
...
function StatRow({
	...
	return (
		<div style={{ display: "flex", flexDirection: "column" }}>
			...
			{expandable && expanded && children && <div style={childrenContainerStyle}>{children}</div>}
		</div>
	);
}
		...
		<div style={outerStyle}>
			<div style={cardStyle}>
				<div style={{ padding: 12 }}>
					<button onClick={() => setIsExpanded(!isExpanded)} style={headerBtnStyle} type="button">
						{isExpanded ? (
						...
						>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
								...
								<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
									<StatRow
										count={memoryCount}
										icon={<HexagonIcon fill={colors.memFill} stroke={colors.memStrokeDefault} />}
										label="Memories"
										colors={colors}
									/>
									<StatRow
							...
							<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
								...
								<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
									<div style={statusRowStyle}>
										<HexagonIcon fill={colors.memFill} stroke={colors.memBorderRecent} />
										<span style={edgeLabelStyle}>Recent (&lt; 24h)</span>
									</div>
									<div style={statusRowStyle}>
										<HexagonIcon fill={colors.memFill} stroke={colors.memBorderExpiring} />
										<span style={edgeLabelStyle}>Expiring soon</span>
									</div>
									<div style={statusRowStyle}>
										<HexagonIcon fill={colors.memFill} stroke={colors.memBorderForgotten} />

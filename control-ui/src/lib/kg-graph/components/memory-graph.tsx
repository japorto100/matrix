...
export function MemoryGraph({
	...
	const colors = useMemo<GraphThemeColors>(
		() => (colorOverrides ? { ...resolvedColors, ...colorOverrides } : resolvedColors),
	...
	useEffect(() => {
		if (
			...
			const timer = setTimeout(() => {
				viewportRef.current?.fitToNodes(nodes, containerSize.width, containerSize.height);
	...
	const handleViewportChange = useCallback((zoom: number, popoverVisible: boolean) => {
	...
	const navigateDown = useCallback(() => {
		...
		if (node?.type === "document") {
			const child = nodes.find(
				(n) => n.type === "memory" && "documentId" in n.data && n.data.documentId === selectedNode,
			);
	...
	const navigateNext = useCallback(() => {
		...
		if (node.type === "document") {
			...
			const siblings = nodes.filter(
				(n) => n.type === "memory" && "documentId" in n.data && n.data.documentId === docId,
			);
	...
	if (externalError) {
		...
		return (
			<div style={errorContainerStyle}>
				<div style={errorBoxStyle}>Error loading graph: {externalError.message}</div>
			</div>
		);
	}
		...
		<div style={wrapperStyle}>
			...
			<div style={canvasContainerStyle} ref={containerRef}>
				...
				<div>
					...
					<Legend colors={colors} edges={edges} isLoading={isLoading} nodes={nodes} />
				</div>
			</div>
		</div>
	);

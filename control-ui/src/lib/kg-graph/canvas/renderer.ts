...
function edgeStyle(
	...
	if (edge.edgeType === "derives") return { color: colors.edgeDerives, width: 1.2, opacity: 0.4 };
	if (edge.edgeType === "updates") return { color: colors.edgeUpdates, width: 2, opacity: 0.7 };
	// "extends" and any unknown edge types
	return { color: colors.edgeExtends, width: 1.5, opacity: 0.55 };
}

function batchKey(style: { color: string; width: number; opacity: number }): string {
...
function drawEdges(
	...
	for (const edge of edges) {
		...
		if (hasDim) {
			...
			connected = srcId === state.selectedNodeId || tgtId === state.selectedNodeId;
		}
		...
		prepared.push({
			...
			arrowSize: edge.edgeType === "updates" ? Math.max(6, 8 * viewport.zoom) : 0,
		});
	}
	...
	for (const [key, batch] of edgeBatches) {
		...
		if (!isDimmed) {
			const glowAlpha =
				batchEdgeType === "updates" ? first.style.opacity * 0.4 : first.style.opacity * 0.3;
			const glowWidth =
				batchEdgeType === "updates" ? first.style.width + 2 : first.style.width + 1.5;
			ctx.save();
		...
		ctx.globalAlpha = isDimmed ? baseAlpha * (1 - state.dimProgress * 0.8) : baseAlpha;
		ctx.strokeStyle = first.style.color;
...
function drawNodes(
	...
	for (const node of nodes) {
		...
		if (isSelected || isHighlighted || isHovered) {
			drawGlow(ctx, screen.x, screen.y, screenSize, node.type, colors, isHovered && !isSelected);
		}
	}

	const dimAlpha = state.selectedNodeId && state.dimProgress > 0 ? 1 - state.dimProgress * 0.7 : 1;
...
function drawDocumentNode(
	...
	ctx.strokeStyle = isSelected || isHighlighted || isHovered ? colors.accent : colors.docStroke;
	...
	const docType = node.type === "document" ? (node.data as DocumentNodeData).type : "text";

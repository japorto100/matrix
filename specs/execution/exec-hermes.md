...
| usage_pricing.py | `_ref/hermes-agent/agent/usage_pricing.py` (687 LOC → 180 LOC slim-port) | `python-backend/agent/billing/usage_pricing.py` | **DONE Phase-B P4** (2026-04-20) | `exec-16.md §2.10` |
| insights.py | `_ref/hermes-agent/agent/insights.py` (dual-path) | `python-backend/agent/billing/insights.py` | **DONE Phase-B P4** (2026-04-20) | `exec-16.md §2.10` + `exec-harness.md §4f` |
| model_metadata.py | `_ref/hermes-agent/agent/model_metadata.py` (1116 LOC → 110 LOC LiteLLM-wrapper) | `python-backend/agent/llm/model_metadata.py` | **DONE Phase-B P4** (2026-04-20) | `exec-16.md §3.1` |
| context_compressor.py | `_ref/hermes-agent/agent/context_compressor.py` | split → `python-backend/agent/middleware/compaction.py` + `compression.py` | **DONE Phase-B P5** (2026-04-20) | `exec-context.md §11` + `exec-memory.md §3h` |
| title_generator.py | `_ref/hermes-agent/agent/title_generator.py` | backend fallback: `python-backend/agent/titles/generator.py` + migration 024. **Primary target: `exec-transformersjs.md §3.5` (browser-side local model)** | **Backend-fallback DONE** (2026-04-20); **primary transformers.js impl PENDING** | `exec-transformersjs.md §3.5` (primary) + `exec-06.md §4d` (legacy/fallback) |
| manual_compression_feedback.py | `_ref/hermes-agent/agent/manual_compression_feedback.py` | `frontend_merger/src/features/agent/CompressionFeedback.tsx` | **PENDING frontend** | `exec-06.md §4c.b` |
...
|---|---|---|
| Hybrid Agno-Loop | `_ref/hermes-agent/environments/agent_loop.py` → `python-backend/agent/runners/simple.py` + `dispatcher.py` + migration 025 | **DONE Phase-C** (2026-04-20, opt-in via `AGENT_SIMPLE_LOOP_PCT`) |
...
- [ ] **§4.1 Cron-Scheduler** — **Ownership transferred to `exec-scheduler.md`** (2026-04-18). Der originale Hermes-Port-Plan (Python + fcntl) ist obsolet; matrix-Adoption geht in Go (River + Postgres + NATS-dispatch) und deckt 15 use-cases über 4 Kategorien. Sieh `exec-scheduler.md §4 Architektur` + `§13 Phase Plan`. **Nicht blockiert** — Phase 1 ~1 Woche wenn priorisiert.
...
| 2026-04-20 (Phase-C tail — commit `f027e8f`) | **End-to-end A/B data flow activated.** Scheduled harness-backfill worker shipped (`exec-scheduler.md §8.1`). Go `HarnessBackfillWorker` in `go-appservice/internal/scheduler/` calls Python `POST /internal/harness/backfill` every 15 min (cron `*/15 * * * *`, overridable via `Config.HarnessBackfillSchedule`). Python endpoint polls `agent.ab_experiments WHERE harness_fitness_score IS NULL AND finished_at IS NOT NULL`, calls `score_session(thread_id)` per row, returns `{scored, skipped}`. Idempotent by construction. `HARNESS_BACKFILL_LIMIT=200` default per tick. Closes the last blocker — Phase-C A/B analysis now has real quality signals flowing into the ab_experiments table without manual intervention. exec-harness §4g.4 scheduled-scorer TODO marked done. |
| 2026-04-20 (Phase-C) | **Hybrid Agent Loop A/B shipped.** (1) Migration 025 `agent.ab_experiments` — client-UUID primary key (Contrarian-CRITICAL-2 fix: fire-and-forget INSERT, no RETURNING, no hot-path DB latency), per-experiment/variant/user/session indexes, `harness_fitness_score NUMERIC NULL` + `harness_eval_id TEXT NULL` als quality-signal-slot (Contrarian-CRITICAL-3: harness-scorer populates statt selbstgebauter suspected_retry heuristik). (2) `agent/runners/simple.py` — port von `_ref/hermes-agent/environments/agent_loop.py`, pure-asyncio loop, explizit NICHT in `agent/graph/` weil nicht graph-basiert. **Hook-drift resolution (Contrarian-MAJOR-1):** reuses `llm_node(state)` + `tool_node(state)` direkt als plain async functions statt duplication — jede Phase-B-hook (CredentialPool, rate-limit, cost span-attrs, redact, classify_error) läuft automatisch mit. `_prepare_system_prompt` + `_prepare_messages` + `_safe_sync_turn` reused via import aus `graph/runner.py`. Streaming DISABLED (Contrarian-MAJOR-3 lock) bis Phase-D frontend-validated. (3) `agent/runners/dispatcher.py` — per-user sticky bucketing (SHA-256 hash mod 100), Valkey-backed kill-switch mit 5s in-process TTL-cache (Contrarian-MAJOR-2 fix: env-var alone = deploy-gated), env-var `AGENT_SIMPLE_LOOP_PCT` controls rollout (default 0 → 100% LangGraph). **No silent fallback (Contrarian-BLOCKER-1 fix):** SimpleLoop exception emits clean ErrorPacket instead of starting second LangGraph stream (avoids AI-SDK double-TextStart-packet rendering ambiguity). Fire-and-forget UUID-keyed INSERT + UPDATE; row-loss-on-crash acceptable for experiment table. (4) `agent/app.py` — `run_agent_loop` call replaced with `run_agent_loop_with_variant`; `GET /api/v1/agent/ab/status` ops endpoint reports current percentage + kill-switch state. (5) Quality-signal design: harness/scorer populates `harness_fitness_score` post-turn by joining on `session_id` — meta-harness (`agent/harness/`) liefert das user-satisfaction-signal statt selbstgebauter heuristik. 17 neue unit tests (tests/agent/runners/test_dispatcher.py + test_simple.py). 263 agent tests pass, 0 regressions. |
| 2026-04-20 (Phase-B P6) | **Compression-UI backend + Title-Gen.** (1) Migration 024 `agent.sessions.title TEXT nullable` + index `(user_id, title)` (Contrarian-BLOCKER-2 fix — column wasn't present in migration 016, Control-UI-search needed it). (2) `agent/titles/generator.py` — `generate_title(user_message, assistant_reply, max_words=7)` über `litellm.acompletion`, sanitizer (first-line + iterative quote/punctuation-strip + word-cap) verhindert runaway-LLM-output von DB-row-bloat; **credential-isolation (Contrarian-2 MAJOR-5 enforced)** via dedicated env `MATRIX_TITLE_GEN_KEY` + `MATRIX_TITLE_GEN_MODEL` (default `claude-haiku-4-5-20251001`). Kein CredentialPool, kein user-quota-consumption, nicht in InsightsEngine-billing. Absent env-var → returns None + UI falls back to truncated first-message. `persist_session_title(session_id, title)` direct UPDATE on agent.sessions, fail-soft. (3) `agent/app.py` — `GET /api/v1/agent/context/compression-status?thread_id=X&model=Y` → `{window, thresholds, stage, engine}`. Stateless, cheap, client enriches mit per-turn token-counts. (4) 9 neue unit-tests (tests/agent/titles/test_generator.py). 246 agent tests pass, 0 regressions. Cross-exec-doc-fill: `exec-06.md §4c + §4d` mit backend-done / frontend-pending split. **Pending frontend follow-up:** CompressionIndicator.tsx + CompressionFeedback.tsx + async title-gen dispatch aus runner (out-of-scope für Phase-B python-backend). |
| 2026-04-20 (Phase-B P5) | **Compaction/Compression split + ContextEngine-Router.** (1) `agent/middleware/compaction.py` (NEW) — mechanical pruning: `estimate_tokens` + `offload_large_tool_results` (idempotent via truncation-marker, Contrarian-resistant to repeat-pass) + `compact` umbrella. Pure CPU, no LLM, safe to run per-turn. (2) `agent/middleware/compression.py` (NEW) — LLM-summary path: `notify_pre_compression(messages, user_id, bank_id)` invokes `MemoryManager.on_pre_compress` under `asyncio.wait_for(timeout=0.5)` (overridable via `AGENT_PRE_COMPRESS_TIMEOUT_S`), archive-miss logged and compression continues on timeout/None/exception; `summarize_old_messages` replaces old messages with LLM summary, fall-back to truncation on LLM failure. (3) `context/context_engine.py` — `DefaultContextEngine.stage_for_model(tokens, model)` **additive** (ABC signature permanent stable, Contrarian-2 CRITICAL-1 guarantee). (4) `graph/runner.py::_prepare_messages` — ContextEngine router: `normal/pre_save` no-op; `compaction` → mechanical compact(); `emergency` → span.add_event(`pre_compression`) + compress(user_id, bank_id). Fall-back to legacy `apply_context_management` shim when ContextEngine import fails. (5) `agent/middleware/summarization.py` — jetzt deprecation-shim, delegiert zu compaction/compression, behält alle legacy-symbols (estimate_tokens, offload_large_tool_results, summarize_old_messages, should_summarize, apply_context_management) für 1-2 release cycles. 11 neue unit tests (tests/agent/middleware/test_compaction_compression.py). 236 agent tests pass, 0 regressions. Cross-exec-doc-fill: `exec-context.md §11 + §13` (router, two-tier consumer contract, model-window integration); `exec-memory.md §3h` (ABC contract enforced, concrete MemPalace/Hindsight impls deferred to follow-up tickets). |
| 2026-04-20 (Phase-B P4) | **Cost-tracking trifecta gelandet.** (1) `agent/billing/usage_pricing.py` — `CanonicalUsage` dataclass (1:1 hermes) + `usage_from_litellm` normalisiert OpenAI/Anthropic/Google-usage-shapes (inkl. cache-read, cache-write, reasoning tokens) + `estimate_usage_cost(model, usage)` dreistufig: LiteLLM primary → snapshot fallback (flagship-only) → unknown. 687 LOC hermes → 180 LOC matrix (keine OAuth-refresh, keine SQLite-state, keine `_OFFICIAL_DOCS_PRICING` dict). (2) `agent/billing/insights.py` — `InsightsEngine(conn)` liest `agent.spans` JSONB (NICHT SQLite). `generate(user_id, days)` aggregiert per-user + `cost_for_session(session_id)` für harness. Dual-path: billing-REST (exec-16 next phase) + harness/scorer. **Tier-1 redact per-span in aggregation** (Contrarian-2 MAJOR-4 fix, prevents API-leak wenn Tier-2 async consumer lagging). `InsightsReport` enthält per_model_cost + per_model_tokens, JSON-serialisierbar via `to_json()`. (3) `agent/llm/model_metadata.py` — LiteLLM wrapper mit 1h TTL-cache, `get_model_info`/`get_model_context_window`/`normalize_model_id`. 1116 LOC hermes → 110 LOC matrix (kein sync `requests.get()` beim import). (4) `llm_node.py` — post-LLM-call emit `llm.input_tokens`/`llm.cache_read_tokens`/`llm.cache_write_tokens`/`llm.reasoning_tokens`/`llm.cost_usd`/`llm.cost_status`/`llm.cost_source` auf `agent.turn` span. Fail-soft — cost-estimation failure never breaks the turn. (5) Drei hardcoded model-lookup-Stellen ersetzt: `middleware/summarization.py::MODEL_MAX_TOKENS` dict entfernt + `get_model_context_window()` call-site; `harness/scorer.py::MODEL_COST_PER_MTOK` dict entfernt + `estimate_usage_cost` delegation (60/40 input/output heuristik für audit-events-to-spans transition); `graph/runner.py::_fallback_model_max_tokens` P1-shim deleted → `get_model_context_window(ctx.model)` direkt. 24 neue unit tests (tests/agent/billing/ + tests/agent/llm/). 225 agent tests pass, 0 regressions. Cross-exec-doc-fill: `exec-16.md §2.10 + §3.1` mit vollem implementation-content. |
...
**Phase-B P4 (Cost-Tracking Trifecta) — 2026-04-20:**

- [x] `agent/billing/usage_pricing.py` — CanonicalUsage + `usage_from_litellm` + `estimate_usage_cost` (LiteLLM-primary / snapshot-fallback / unknown)
- [x] `agent/billing/insights.py` — `InsightsEngine` reading agent.spans JSONB; dual-path (billing-REST + harness scorer); Tier-1 redact applied per-span in aggregation
- [x] `agent/llm/model_metadata.py` — LiteLLM wrapper + 1h TTL cache + `get_model_context_window`
- [x] Cost span-attributes (`llm.cost_usd`, `llm.cost_status`, `llm.cost_source`, `llm.cache_read_tokens`, etc.) emitted in `llm_node.py` post-LLM-call
- [x] 3 hardcoded model-lookup sites replaced: `middleware/summarization.py`, `harness/scorer.py`, `graph/runner.py` (`_fallback_model_max_tokens` shim deleted)
- [x] 24 new unit tests (tests/agent/billing/ + tests/agent/llm/). 225 total agent tests pass, 0 regressions

**Phase-B P5 (Compaction/Compression Split) — 2026-04-20:**

- [x] `agent/middleware/compaction.py` (NEW) — mechanical pruning, idempotent, no LLM
- [x] `agent/middleware/compression.py` (NEW) — LLM summary + bounded `notify_pre_compression` hook
- [x] `DefaultContextEngine.stage_for_model(tokens, model)` — additive convenience on concrete class (ABC signature permanent stable)
- [x] `runner._prepare_messages` ContextEngine-router: compaction-stage → compact(); emergency-stage → span-event + compress()
- [x] `middleware/summarization.py` → deprecation shim delegating to compaction + compression
- [x] 11 new unit tests (tests/agent/middleware/)
- [x] 236 agent tests pass, 0 regressions

**Phase-B P6 (Compression-UI backend + Title-Gen) — 2026-04-20:**

- [x] Migration 024 — `agent.sessions.title` column + search index (Contrarian-BLOCKER-2 fix)
- [x] `agent/titles/generator.py` — remote-LLM title generator with strict sanitisation, credential-isolation via `MATRIX_TITLE_GEN_KEY`
- [x] `agent/titles/generator.py::persist_session_title` — fail-soft UPDATE on agent.sessions
- [x] `GET /api/v1/agent/context/compression-status` — backend endpoint for compression indicator
- [x] 9 new unit tests (tests/agent/titles/)
- [x] 246 agent tests pass, 0 regressions

**Phase-B P6 frontend (deferred as a separate ticket):**

- CompressionIndicator.tsx (status-dot + expand-panel)
- CompressionFeedback.tsx (opt-in "improve this summary")
- Async title-gen dispatch from runner (asyncio.create_task after first assistant reply)

Deferred because it crosses into the frontend-merger track and the python-backend Phase-B scope is complete with the backend contract in place.
...
**Test-Summe (as of Phase-B P6):** 246 agent tests pass, 0 regressions from Phase-1 baseline. Ruff clean on all Phase-B-touched files. 3 new alembic migrations shipped in Phase-B (022 sync_failures, 023 redaction_patterns, 024 sessions.title), all with clean downgrade paths.

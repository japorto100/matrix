...
LOW_RISK_TOOLS: set[str] = {
    "memory_store", "memory_search", "working_memory_set", "working_memory_get",
    "list_tools", "get_portfolio", "get_positions",
}
...
def _load_prompt_guard() -> bool:
    ...
    try:
        ...
        _prompt_guard_model = AutoModelForSequenceClassification.from_pretrained(PROMPT_GUARD_MODEL_ID)
...
def prompt_guard_scan(text: str, threshold: float = 0.85) -> PromptGuardResult:
    ...
    try:
        ...
        if injection_score >= threshold:
            logger.warning("PromptGuard detection: injection (score=%.3f)", injection_score)
            return PromptGuardResult(is_injection=True, score=injection_score, label="injection")

        return PromptGuardResult(is_injection=False, score=injection_score, label="safe")
...
def sanitize_input(tool_name: str, raw_output: str) -> SanitizePipelineResult:
    ...
    content = raw_output if isinstance(raw_output, str) else json.dumps(raw_output, default=str)

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "provider_id", sa.Text, nullable=False
        ),  # anthropic, openrouter, binance
        sa.Column(
            "credential_enc", sa.LargeBinary, nullable=False
    ...
    op.create_index(
        "ix_user_credentials_user", "user_credentials", ["user_id"], schema="agent"
    )
    op.create_index(
        "ix_user_credentials_category", "user_credentials", ["category"], schema="agent"
    )

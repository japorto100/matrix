...
class GoStorageClient:
    ...
    async def get_download_url(self, file_id: UUID | str) -> str:
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            r = await client.get(f"{self.base_url}/api/v1/storage/artifacts/{file_id}/download")
            r.raise_for_status()
            data = r.json()
            url = data.get("url") or data.get("download_url")
            if not url:
                raise ValueError(f"Go gateway returned no signed URL for {file_id}: {data}")
            return url
    ...
    async def patch_metadata(self, file_id: UUID | str, patch: dict[str, Any]) -> dict[str, Any]:

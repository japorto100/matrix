...
- [ ] T034 Decide and implement/defer persisted user model selection.

## Billing And Insights

- [ ] T040 Verify `CanonicalUsage` records prompt/completion/cache/reasoning
  tokens from a real response.
- [ ] T041 Verify cost estimate uses LiteLLM model metadata.
- [ ] T042 Verify span attributes contain cost and usage.
- [ ] T043 Verify InsightsEngine rollup.
...
- [ ] T060 Verify G1 bilingual keyword/tokenizer behavior.
- [ ] T061 Verify G2 credential pre-flight keeps primary when cheap provider
  is unavailable.
- [ ] T062 Verify G3 config cache avoids per-turn DB connection churn.
- [ ] T063 Verify G4 A/B routing dimensions are written.
- [ ] T064 Verify G5 user-visible routing indicator.
- [ ] T065 Verify G6 Control-UI smart-routing toggle/disable path.
- [ ] T066 Verify P1 `router_node` first-turn-only behavior.
- [ ] T067 Fix or accept F-G4 A/B insert/update race.
- [ ] T068 Fix or accept F-G1 keyword quality issue.
- [ ] T069 Document or change F-4g4 scorer eval-id overwrite semantics.
...
- [ ] T072 Keep L3 classifier deferred until corpus and L2 plateau justify it.
- [ ] T073 Keep L4 full A2FM training out of current scope.

...
import { formatFileSize } from "@/lib/matrix/utils";
...
function GenericFileContent({ message }: { message: ResolvedMessage }) {
	return (
		<a href={message.url ?? "#"} target="_blank" rel="noopener noreferrer" download={message.fileName} className="flex items-center gap-3 bg-muted/50 rounded-xl px-3 py-2.5 hover:bg-muted transition-colors no-underline max-w-[280px]">
			...
			<div className="flex-1 min-w-0">
				...
				{message.fileSize !== undefined && <p className="text-[10px] text-muted-foreground">{formatFileSize(message.fileSize)}</p>}
			</div>
...
function DocxContent({ message }: { message: ResolvedMessage }) {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (containerRef.current) containerRef.current.innerHTML = "<p class='p-4 text-sm text-muted-foreground'>Vorschau nicht verfügbar</p>";
			}
		})();
	...
	return (
		<>
			<button type="button" onClick={() => setShowPreview(true)} className="flex items-center gap-3 bg-muted/50 rounded-xl px-3 py-2.5 hover:bg-muted transition-colors max-w-[280px] border-0 cursor-pointer text-left">
			...
			{showPreview && (
				<Dialog open={showPreview} onOpenChange={setShowPreview}>
					<DialogContent className="max-w-[80vw] max-h-[85vh] p-0 border-none overflow-hidden" aria-describedby={undefined}>
			...
			{showPreview && (
				<Dialog open={showPreview} onOpenChange={setShowPreview}>
					<DialogContent className="max-w-[80vw] max-h-[85vh] p-0 border-none overflow-hidden" aria-describedby={undefined}>
						...
						<div className="w-full h-[80vh] overflow-auto bg-white p-4 text-black [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-gray-300 [&_td]:px-2 [&_td]:py-1 [&_td]:text-xs [&_th]:border [&_th]:border-gray-300 [&_th]:px-2 [&_th]:py-1 [&_th]:text-xs [&_th]:bg-gray-100 [&_th]:font-semibold" dangerouslySetInnerHTML={{ __html: tableHtml }} />
					</DialogContent>
				</Dialog>
			)}
		</>
			...
			{showPreview && (
				<Dialog open={showPreview} onOpenChange={setShowPreview}>
					<DialogContent className="max-w-[80vw] max-h-[85vh] p-0 border-none overflow-hidden" aria-describedby={undefined}>
						...
						<iframe src={message.url} title={message.fileName ?? "PDF"} className="w-full h-[80vh] border-0" />
					</DialogContent>
				</Dialog>
			)}
		</>
...
export function FileContent({ message }: { message: ResolvedMessage }) {
	...
	if (mime === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || ext === "docx") return <DocxContent message={message} />;
	if (mime === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" || ext === "xlsx" || ext === "xls" || ext === "csv") return <XlsxContent message={message} />;

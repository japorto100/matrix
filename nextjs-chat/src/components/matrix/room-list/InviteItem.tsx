...
import { hashColor } from "@/lib/matrix/utils";
...
export function InviteItem({ room, client, onSelect }: Props) {
	...
	const handleDecline = async () => {
		...
		} catch { toast.error("Ablehnen fehlgeschlagen."); }
	};
	...
	return (
		<div className="flex items-center gap-3 px-2.5 py-2 rounded-lg bg-primary/5 border-l-2 border-primary">
			...
			<div className="flex items-center gap-1 shrink-0">
				<Button variant="ghost" size="icon" className="h-7 w-7 text-emerald-500 hover:bg-emerald-500/20" onClick={handleAccept} title="Annehmen">
					<Check className="h-4 w-4" />
				</Button>
				<Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/20" onClick={handleDecline} title="Ablehnen">
					<X className="h-4 w-4" />
				</Button>
			</div>
		</div>

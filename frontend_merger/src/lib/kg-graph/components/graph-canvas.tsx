...
export const GraphCanvas = memo<ExtendedGraphCanvasProps>(function GraphCanvas({
	...
	useLayoutEffect(() => {
		...
		const vp = new ViewportState(cfg.initialPanX, cfg.initialPanY, cfg.initialZoom);
		viewportRef.current = vp;
		if (externalViewportRef) {
			(externalViewportRef as React.MutableRefObject<ViewportState | null>).current = vp;
		}
	...
	useEffect(() => {
		...
		const tick = () => {
			...
			if (!vpMoving && !simActive && !dimming && !spatialChanged && !renderNeeded.current) return;
			renderNeeded.current = false;

			// Report viewport changes so zoom display and popover positions update.
			const vpChanged = vp.panX !== prevVpX || vp.panY !== prevVpY || vp.zoom !== prevVpZoom;
			if ((vpChanged || simActive) && cb.current.onViewportChange) {
				cb.current.onViewportChange(vp.zoom, !!(cur.selectedNodeId || cur.hoveredNodeId));
			}
			if (vpChanged) {
				prevVpX = vp.panX;
				prevVpY = vp.panY;
